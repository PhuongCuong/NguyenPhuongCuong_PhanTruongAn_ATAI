...
import { cartDelete, uploadcart } from "../redux/cartSlice";
...
const Itemcart = (props) => {
  ...
  const handlepuls = (items) => {
    let updatecart = cart.map((item) =>
      item.id === items.id && item.portfolio === items.portfolio
  ...
  const handleDelete = (items) => {
    let updatecart = cart.filter(
      (item) => !(item.id === items.id && item.portfolio === items.portfolio)
    );
    if (updatecart.length === 0) {
      updatecart = [];
    }
    dispath(cartDelete(user));
    dispath(uploadcart(updatecart));
  };
